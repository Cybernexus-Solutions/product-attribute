...
class ProductTemplate(models.Model):
    ...
    def _calc_volume(self, product_length, product_height, product_width, uom_id):
        ...
        if product_length and product_height and product_width and uom_id:
            length_unit = self.convert_to_unit(product_length, uom_id)
            height_unit = self.convert_to_unit(product_height, uom_id)
            width_unit = self.convert_to_unit(product_width, uom_id)
            volume = length_unit * height_unit * width_unit
    ...
    def convert_to_unit(self, measure, dimensional_uom):
        product_length_in_feet = self.env["ir.config_parameter"].sudo().get_param("product.volume_in_cubic_feet")
        uom_unit = self.env.ref("uom.product_uom_foot") if product_length_in_feet else self.env.ref("uom.product_uom_meter") 
